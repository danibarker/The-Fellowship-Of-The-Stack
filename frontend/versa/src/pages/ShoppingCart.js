...
const ShoppingCart = () => {
  ...
  const calcCartTotal = () => {
    ...
    const total = cartItems.reduce((total, cartItem) => total + cartItem.itemPrice * cartItem.itemQuantity, 0);
    return total.toFixed(2);
  };
  const calcNumItems = () => {
    if (!cartItems) return 0;
    const total = cartItems.reduce((total, cartItem) => total + cartItem.itemQuantity, 0);
    return total;
  };
