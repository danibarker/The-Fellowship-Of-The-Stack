...
const ProductPage = ({
  ...
  const showProductTotal = () =>
    ...
    cart &&
    cart[id] &&
    Object.keys(cart[id]).reduce(
      (colourTotal, currColour) =>
        colourTotal +
        Object.keys(cart[id][currColour]).reduce(
          (sizeTotal, currSize) => sizeTotal + cart[id][currColour][currSize],
          0
        ),
      0
    );
  const getMainImageSrc = () => {
    ...
    if (images && images.length > 0) {
      return `https://versabucket.s3.us-east-2.amazonaws.com/images/${
  ...
  return (
    <Container>
      ...
      <MainInfo>
        <ProductImages>
          <MainImage src={getMainImageSrc()} alt="image" />
          <OtherImages>
            {images &&
              images.length > 0 &&
        ...
        <ProductDetail>
          ...
          {colours && colours.length > 0 && (
            <Colours>
              <SelectedColour>
                ...
                {stock.map((variation) => {
                  if (
                    variation.color === colours[choices.colour].label &&
          ...
          {sizes && sizes.length > 0 && colours && colours.length > 0 && (
            <ClearSelection
              onClick={() => {
            ...
            >
              <LineCloseIcon stroke={theme.primary} width="26" height="26" />
              Clear Selection
            </ClearSelection>
          )}
          <Description>
          ...
          <Materials>
            <h3>Materials</h3>
            <p>{materials || "Loading materials..."}</p>
          </Materials>
          ...
          >
            {clicked ? (
              <CheckMarkIcon height="20px" stroke={theme.secondary} />
            ) : (
