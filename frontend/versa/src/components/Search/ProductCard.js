...
function calcTotalStock(product) {
  return product.stock.reduce((total, curr) => total + curr.quantity, 0);
}
...
const ProductCard = ({ product }) => {
  ...
  const getProductImage = () => {
    if (product.image) {
      return `${host}/images/${product.image}.jpeg`;
    }
    if (product.thumbnail) {
      return `https://versabucket.s3.us-east-2.amazonaws.com/images/${product.thumbnail}.jpeg`;
    }
    return "";
  };
  return (
    <Link to={`/product-item/${product.id}`}>
      ...
      <ItemCard title={product.title} price={product.price}>
        {product.stock.reduce((total, curr) => total + curr.quantity, 0) ===
          0 && (
          <OOSPill>
            <p>Out of Stock</p>
          </OOSPill>
