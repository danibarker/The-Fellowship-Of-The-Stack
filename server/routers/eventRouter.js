...
router.get("/myArtistsEvents/", auth, async (req, res) => {
  try {
    ...
    for (result of results) {
      ...
      let startDate = resultsStartDate.toLocaleDateString("en-US", options);
...
router.get("/allEvents", async (req, res) => {
  try {
    const result =
      await client.query(`SELECT u.username as host_name, i.sum AS num_interested, a.sum AS num_attending, e.* from (SELECT event_id, SUM ( CASE WHEN status = 'interested'
        THEN 1 ELSE 0 end) FROM events_attendees
        GROUP BY event_id) i
        INNER JOIN
        events e
...
router.post("/join", auth, async (req, res) => {
  ...
  const attResponse = await client.query(
...
router.get("/collabs/:eventid", async (req, res) => {
  try {
    const collabs = await client.query(
