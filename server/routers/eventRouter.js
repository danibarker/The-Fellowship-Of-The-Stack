...
router.post("/join", auth, async (req, res) => {
  ...
  const attResponse = await pool.query(
    `SELECT h.username as host_name, e.name as event_name, 
    e.description, e.start_time, e.end_time, e.location, a.event_id,
    u.email, u.name from events_attendees a
    INNER JOIN users u ON a.attendee = u.id
    INNER JOIN events e ON e.id=a.event_id
    INNER JOIN users h ON h.id=e.host WHERE a.event_id = ${eventID} and u.id = ${req.user.id}`
  );
  const attendee = attResponse.rows[0];
...
router.get("/not-attending/email/:eventid/:id", async (req, res) => {
  try {
    const attendees = await pool.query(
      `SELECT h.username as host_name, e.name as event_name, e.description,
