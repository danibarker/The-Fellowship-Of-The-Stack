...
router.get("/get", async (req, res) => {
  try {
    const result =
      await pool.query(`SELECT m.*, ut.name as to_name, uf.name as from, 
      ut.username as to_username, 
      uf.username as from_username FROM messages m
    ...
    results.forEach((message) => {
      [message.to_name] = message.to_name.split(" ");
      [message.from_name] = message.from_name.split(" ");
    });
    res.json(results);
  } catch (e) {
    console.log(e, "/get");
