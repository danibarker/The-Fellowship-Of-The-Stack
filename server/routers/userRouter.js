...
router.post("/create", async (req, res) => {
  ...
  const data = await pool.query(
    `INSERT INTO users (username, password, email, address, is_artist, is_driver, name) 
...
router.put("/update", auth, async (req, res) => {
  ...
  if (req.body.data.isArtist === false || req.body.data.isArtist === true) {
    updatedUser.isArtist = req.body.data.isArtist;
  } else {
    updatedUser.isArtist = user.is_artist;
  }
  if (req.body.data.isDriver === false || req.body.data.isDriver === true) {
    updatedUser.isDriver = req.body.data.isDriver;
  } else {
    updatedUser.isDriver = user.is_driver;
  }

  // now we need to change whats in the updatedUser array, this does the update
  const data = await pool.query(
    `UPDATE users SET username=$1, email=$2, address=$3, store_address=$4, 
    is_artist=$5, is_driver=$6, name=$7 
