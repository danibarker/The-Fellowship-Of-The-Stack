...
router.get("/driver/order-to-fulfill", auth, async (req, res) => {
  try {
    let deliveries = await client.query(
...
router.get("/driver/order-to-fulfill/:orderid", async (req, res) => {
  try {
    const singleDelivery = await client.query(`
...
router.get("/driver/past/:orderid", auth, async (req, res) => {
  try {
    const pastDeliveryItems = await client.query(
      `
...
router.get("/driver/assigned-pickups", auth, async (req, res) => {
  try {
    const assignedDeliveries = await client.query(
      `
...
router.get("/driver/ready-deliveries", auth, async (req, res) => {
  try {
    const resDeliveries = await client.query(
      `
    ...
    const resIncompleteOrders = await client.query(
    ...
    const completedDeliveries = (incompleteOrdersIDs, deliveriesIDs) => {
      ...
      return Array.from(new Set([...diff(deliveriesIDs, incompleteOrdersIDs)]));
    };
...
router.put("/driver/deliveries/update/:orderid", auth, async (req, res) => {
  try {
