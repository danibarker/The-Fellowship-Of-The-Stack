...
router.get("/:session", optionalAuth, async (req, res) => {
  try {
    ...
    const itemsQuery = await pool.query(
      `SELECT s.quantity AS num_left, C.*, p.thumbnail, p.sizes, p.title, p.price 
        FROM cart_items C 
...
router.post("/add", optionalAuth, async (req, res) => {
  ...
  if ((await checkForItem).rows.length > 0) {
    ...
    pool.query(
      `INSERT INTO cart_items (product_id, colour, size, quantity,cart_id) 
      values (${cartProduct},'${colour}','${size}',${quantity},${cartID}) `
    );
  }
...
router.put("/edit", optionalAuth, async (req, res) => {
  ...
  await pool.query(`UPDATE cart_items SET quantity = ${quantity}
    WHERE product_id=${cartProduct} AND colour='${colour}' 
    AND size='${size}' AND cart_id='${cartID}'`);
  res.status(200).send();
});

router.delete("/remove", optionalAuth, async () => {
