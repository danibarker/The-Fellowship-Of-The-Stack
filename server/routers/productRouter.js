...
router.get("/search/:searchQuery", async (req, res) => {
  ...
  query.forEach((term, index) => {
    if (index === 0) {
      ...
      queryString += ` AND (UPPER (title) LIKE '%${term}%' OR 
      UPPER (description) LIKE '%${term}%')`;
    }
  });
...
router.post("/create", auth, async (req, res) => {
  if (!req.user.is_artist) {
    ...
    try {
      const { title, price, description, materials } = req.body.data;
      let { colours, sizes } = req.body.data;
      ...
      const productInfo = await pool.query(
        `INSERT INTO products (title, price, description, colours, artist_id, sizes, materials) 
...
router.put("/edit/:id", auth, async (req, res) => {
  if (!req.user.is_artist) {
    ...
    try {
      let { colours, sizes } = req.body.data; // For use in set
      const { title, price, description, materials, status } = req.body.data;
      if (colours.length === 0) {
        colours = [{ label: "O", value: "#444" }];
      }
      if (sizes.length === 0) {
      ...
      const response = await pool.query(
        `UPDATE products SET title = $1, price = $2,
        description = $3, colours = $4, sizes = $5, 
        materials = $6, status = $7 WHERE id = $8 
        RETURNING id`,
        [
          title,
          price,
      ...
      if (status === "Backorder" || status === "Discontinue") {
        pool.query(`UPDATE stock set quantity = 0 where product_id =${id}`);
      }
